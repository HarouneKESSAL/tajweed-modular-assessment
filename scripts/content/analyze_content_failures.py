...
def main() -> None:
    ...
    length_summary = {
        bucket: summarize_rows(group)
        for bucket, group in sorted(length_groups.items())
    }
    edit_summary = {
        bucket: summarize_rows(group)
        for bucket, group in sorted(edit_groups.items(), key=lambda item: item[0])
    }
