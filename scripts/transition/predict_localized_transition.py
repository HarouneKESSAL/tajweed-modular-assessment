...
def find_matching_indices(rows, target_label: str):
    ...
    if target_label == "none":
        return [i for i, row in enumerate(rows) if not row_labels(row)]
    return [i for i, row in enumerate(rows) if target_label in row_labels(row)]
...
def main():
    ...
    parser.add_argument("--decoder-config", default="checkpoints/localized_transition_decoder.json")
    ...
    if args.find_label:
        ...
        if args.match_offset < 0 or args.match_offset >= len(matches):
            raise IndexError(
                f"match-offset out of range for label '{args.find_label}': 0 <= idx < {len(matches)}"
            )
    ...
    gold_spans = {
        label: [span for span in row.get("transition_rule_time_spans", []) if span.get("rule") == label]
        for label in label_vocab
    }
