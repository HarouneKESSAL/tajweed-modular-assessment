...
def main() -> None:
    ...
    parser.add_argument(
        "--feature-cache-dir",
        default="data/interim/retasy_mfcc_cache",
    )
    parser.add_argument(
        "--checkpoint",
        default="checkpoints/real_duration_classifier.pt",
    )
    ...
    # Split by reciter to reduce leakage across the same speaker
    ...
    pos_weight = compute_pos_weight([train_ds.rows[i] for i in range(len(train_ds.rows))], label_vocab)
    ...
    for label, f1, support in zip(label_vocab, final_metrics["per_label_f1"], final_metrics["support"]):
        print(f"  {label:15s} f1={f1:.3f} support={int(support)}")
