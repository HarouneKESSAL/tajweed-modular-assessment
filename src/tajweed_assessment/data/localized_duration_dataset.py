...
DEFAULT_LOCALIZED_DURATION_LABEL_VOCAB = ["ghunnah", "madd"]
...
def normalize_duration_label(span: Dict[str, Any]) -> str | None:
    coarse = str(span.get("coarse_group") or "").strip()
    rule = str(span.get("rule") or "").strip()
    if coarse == "ghunnah" or rule == "ghunnah":
        return "ghunnah"
    if coarse == "has_madd" or rule.startswith("madd"):
        return "madd"
    return None
...
def build_label_vocab(rows: Sequence[Dict[str, Any]]) -> List[str]:
    ...
    for row in rows:
        for span in row.get("duration_rule_time_spans", []):
            label = normalize_duration_label(span)
            if label:
                labels.add(label)
    return sorted(labels) if labels else list(DEFAULT_LOCALIZED_DURATION_LABEL_VOCAB)
...
class LocalizedDurationDataset(Dataset):
    def __init__(
        self,
    ...
    def _cache_path(self, row: Dict[str, Any]) -> Optional[Path]:
        ...
        return self.feature_cache_dir / f"{row['id']}_{speed_tag}_localized_duration.pt"
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        spans = list(row.get("duration_rule_time_spans", []))
        ...
        return {
            ...
            "duration_labels": row.get("projected_duration_labels") or row.get("gold_duration_labels") or [],
            "x": x,
...
def collate_localized_duration_batch(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
