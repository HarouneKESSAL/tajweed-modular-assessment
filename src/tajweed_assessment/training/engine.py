...
from tajweed_assessment.training.metrics import classification_accuracy, phoneme_sequence_accuracy
...
def train_duration_epoch(model, loader, optimizer, loss_fn: DurationLoss, device: str = "cpu") -> TrainResult:
    model.train()
    total_loss = total_ctc = total_rule = total_acc = 0.0
    for batch in loader:
        x = batch["x"].to(device)
        ...
        losses = loss_fn(log_probs, rule_logits, phoneme_targets, input_lengths, target_lengths, rule_targets)
        ...
        total_acc += phoneme_sequence_accuracy(log_probs.detach().cpu(), batch["input_lengths"], batch["raw_phoneme_targets"])
    n = len(loader)
    return TrainResult(loss=total_loss / n, ctc_loss=total_ctc / n, rule_loss=total_rule / n, phoneme_accuracy=total_acc / n)
...
def train_classifier_epoch(model, loader, optimizer, forward_fn, device: str = "cpu") -> TrainResult:
...
@torch.no_grad()
def evaluate_classifier_epoch(model, loader, forward_fn, device: str = "cpu") -> TrainResult:
    loss_fn = torch.nn.CrossEntropyLoss()
    model.eval()
    total_loss = total_acc = 0.0
    for batch in loader:
